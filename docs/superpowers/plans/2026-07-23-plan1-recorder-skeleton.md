...
- **Execution-time correction (Task 6):** the original Task 6 code stored `cpal::Stream` directly in `Capture`, which (a) deadlocked `stop()` — the callback's `Sender` clone lived inside `_stream`, a field of `self`, so the writer's channel never closed while `join()` blocked — and (b) made `Capture: !Send` on macOS, which would have broken Task 7's `app.manage(AppState)`. As built, the stream is owned by a dedicated thread (startup result reported over a one-shot channel; a dropped `stop_tx` signals shutdown), `stop()` joins the stream thread *before* closing the buffer channel and joining the writer, the level channel is bounded(8) with `try_send`, and a `writer_failed: Arc<AtomicBool>` flags mid-session WAV errors. `Capture`'s public API (`start`/`stop`/`paused`/`level_rx`/`wav_path`) is unchanged, so Task 7 proceeds as written.
